...
class BaselineGTrans_WOLSP(MessagePassing):

    def __init__(self, n_heads=2, d_input=6, d_k=6, alpha=0.9, patch_layer=1, res=1, **kwargs):
        super(BaselineGTrans_WOLSP, self).__init__(aggr='add', **kwargs)
        self.n_heads = n_heads
        # self.dropout = nn.Dropout(dropout)
        self.patch_layer = patch_layer
        self.res = res
...
class BaselineGTrans_WO3Edge(MessagePassing):
    def __init__(self, n_heads=2, d_input=6, d_k=6, alpha=0.9, patch_layer=1, res=1, **kwargs):
        super(BaselineGTrans_WO3Edge, self).__init__(aggr='add', **kwargs)
        self.n_heads = n_heads
        # self.dropout = nn.Dropout(dropout)
        self.patch_layer = patch_layer
        self.res = res
...
class BaselineHPG_v2_WOHie(nn.Module):
    def __init__(self, args, supports=None):
        super(BaselineHPG_v2_WOHie, self).__init__()
        d_model = args.hid_dim
        self.device = args.device
        self.hid_dim = args.hid_dim
        self.N = args.ndim
...
class BaselineHPG_v2_WOLSP(nn.Module):
    def __init__(self, args, supports=None):
        super(BaselineHPG_v2_WOLSP, self).__init__()
        d_model = args.hid_dim
        self.device = args.device
        self.hid_dim = args.hid_dim
        self.N = args.ndim
        ...
        for l in range(self.n_layer):
            # self.gcs.append(UA_GTrans(1, nodevec_dim + enc_dim + args.te_dim, nodevec_dim + enc_dim + args.te_dim, self.alpha))
            self.gcs.append(BaselineGTrans_WOLSP(args.nhead, d_model, d_model, self.alpha, args.patch_layer, self.res))
...
class BaselineHPG_v2_WO3Edge(nn.Module):
    def __init__(self, args, supports=None):
        super(BaselineHPG_v2_WO3Edge, self).__init__()
        d_model = args.hid_dim
        self.device = args.device
        self.hid_dim = args.hid_dim
        self.N = args.ndim
        ...
        for l in range(self.n_layer):
            # self.gcs.append(UA_GTrans(1, nodevec_dim + enc_dim + args.te_dim, nodevec_dim + enc_dim + args.te_dim, self.alpha))
            self.gcs.append(BaselineGTrans_WO3Edge(args.nhead, d_model, d_model, self.alpha, args.patch_layer, self.res))
...
class BaselineHPG_v2_WODTDV(nn.Module):
    def __init__(self, args, supports=None):
        super(BaselineHPG_v2_WODTDV, self).__init__()
        d_model = args.hid_dim
        self.device = args.device
        self.hid_dim = args.hid_dim
        self.N = args.ndim
...
class BaselineHPG_v2_WOTEAGG(nn.Module):
    def __init__(self, args, supports=None):
        super(BaselineHPG_v2_WOTEAGG, self).__init__()
        d_model = args.hid_dim
        self.device = args.device
        self.hid_dim = args.hid_dim
        self.N = args.ndim
